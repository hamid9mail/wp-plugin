 ...
 * ماژول محتوای تعاملی یکپارچه (Interactive Content Module - Enhanced Integratio
n Edition)
 *
 ...
 *
 * قابلیت‌ها:
 * - شورت‌کد [psych_content_block]: برای ساخت بلاک‌های محتوای چندمرحله‌ای (SPA-l
ike).
 * - شورت‌کد [psych_content_view]: برای تعریف "نما" یا "وضعیت"‌های مختلف یک بلا
ک.
 * - شورت‌کد [psych_button]: دکمه‌های هوشمند برای تکمیل ماموریت، باز کردن مودال
، و toggle محتوا.
...
final class Psych_Interactive_Content_Module {
    ...
    private $assets_injected = false; // برای جلوگیری از تزریق چندباره استایل و
اسکریپت
    ...
    private function add_hooks() {
        ...
        // هوک‌های AJAX
        add_action('wp_ajax_psych_load_modal_content', [$this, 'ajax_load_modal_
content']);
        add_action('wp_ajax_nopriv_psych_load_modal_content', [$this, 'ajax_load
_modal_content']);
...
;
        ...
        add_filter('psych_interactive_can_access_content', [$this, 'coach_access
_filter'], 10, 3);
    }
        ...
        if ($context['is_impersonating'] && class_exists('Psych_Coach_Module'))
{
            ...
            $coach_allowed_pages = get_user_meta($coach_id, 'psych_coach_allowed
_pages', true) ?: [];
            if (!user_can($coach_id, 'manage_options') && !in_array($current_pag
e_id, (array)$coach_allowed_pages)) {
                return false;
            }
        }
    ...
    private function register_shortcodes() {
        add_shortcode('psych_content_block', [$this, 'render_content_block_short
code']);
        add_shortcode('psych_content_view', [$this, 'capture_content_view_shortc
ode']);
        add_shortcode('psych_button', [$this, 'render_button_shortcode']);
        add_shortcode('psych_hidden_content', [$this, 'render_hidden_content_sho
rtcode']);
        add_shortcode('psych_progress_path', [$this, 'render_progress_path_short
code']);
    }
    ...
    public function render_content_block_shortcode($atts, $content = null) {
        ...
        if ($atts['require_login'] === 'true' && !$user_id) {
            return '<div class="psych-login-required"><p>برای مشاهده این محتوا
باید وارد شوید.</p></div>';
        }

        // Check access permissions
        $can_access = apply_filters('psych_interactive_can_access_content', true
, $user_id, $atts);
        if (!$can_access) {
            return '<div class="psych-access-denied"><p>شما مجاز به مشاهده این
محتوا نیستید.</p></div>';
        }
        ...
        if (class_exists('PsychoCourse_Path_Engine_2')) {
            ...
            if (method_exists($path_engine, 'get_user_station_for_block')) {
                $current_station = $path_engine->get_user_station_for_block($blo
ck_id, $user_id);
            }
        }
        ...
        if ($atts['type'] === 'quiz' && !empty($this->blocks[$block_id]['seo_dat
a']['questions'])) {
...
>',
        ...
        if (isset($this->blocks[$block_id]['views'][$current_station])) {
            echo do_shortcode($this->blocks[$block_id]['views'][$current_station
]['content']);
        } else {
            $default_content = $this->blocks[$block_id]['views'][$atts['default_
station']]['content'] ??
...
[
    ...
    public function render_button_shortcode($atts, $content = null) {
        ...
        switch ($atts['action']) {
            case 'mission':
                $data_attrs = sprintf('data-action="mission" data-mission-id="%s
"', esc_attr($atts['mission_id']));
                break;
            case 'modal':
                $data_attrs = sprintf('data-action="modal" data-target="#%s"', e
sc_attr($atts['target']));
                break;
            case 'toggle':
                $data_attrs = sprintf('data-action="toggle" data-target="#%s"',
esc_attr($atts['target']));
        ...
        $icon = !empty($atts['icon']) ? sprintf('<i class="%s"></i> ', esc_attr(
$atts['icon'])) : '';
        $button_text = $context['is_impersonating'] ? $atts['text'] . ' (مربی)'
: $atts['text'];
    ...
    public function render_hidden_content_shortcode($atts, $content = null) {
        ...
        // Check access level
        $can_access = $this->check_access_level($atts['access_level'], $user_id,
 $context);
        if (!$can_access) {
            return '<div class="psych-access-denied"><p>شما مجاز به مشاهده این
محتوا نیستید.</p></div>';
        ...
        if ($atts['type'] === 'modal') {
            return sprintf(
                '<div id="%s" class="psych-modal-content" data-title="%s" style=
"display:none;">%s</div>',
                $content_id,
                esc_attr($atts['title']),
                do_shortcode($content)
            ...
            return sprintf(
                '<div id="%s" class="psych-toggle-content" style="display:none;"
>%s</div>',
                $content_id,
                do_shortcode($content)
            );
    ...
    public function render_progress_path_shortcode($atts, $content = null) {
        ...
        if ($context['is_impersonating']) {
            ...
            echo '<div class="coach-progress-notice">
                    ...
                    مسیر پیشرفت برای: <strong>' . esc_html($viewed_user_data->di
splay_name) . '</strong>
                  </div>';
        }
        ...
        if ($atts['type'] === 'badges') {
            echo $this->render_badges_progress($user_id, $atts['show_locked'] ==
= 'true');
        ...
        switch ($access_level) {
            case 'admin':
                return user_can($user_id, 'manage_options') ||
                       ($context['is_impersonating'] && user_can($context['real_
user_id'], 'manage_options'));
            case 'coach':
                if ($context['is_impersonating']) {
                    $coach_roles = ['coach', 'adviser', 'administrator'];
                    $coach_user = get_userdata($context['real_user_id']);
                    return $coach_user && !empty(array_intersect($coach_roles, (
array)$coach_user->roles));
                }
                return false;
            case 'user':
                return $user_id > 0;
...
;
        ...
        foreach ($badges_data as $badge) {
            $status_class = $badge['earned'] ? 'earned' : ($show_locked ? 'locke
d' : 'hidden');
...
>
    ...
    private function render_quiz_schema($block_id, $current_station) {
        ...
        echo '<script type="application/ld+json">' . wp_json_encode($schema, JSO
N_UNESCAPED_UNICODE) . '</script>';
    }
    ...
    public function ajax_load_modal_content() {
        ...
        // Here you would typically load the modal content from database or othe
r source
        // For now, we'll return a simple response
        $content = '<p>محتوای مودال در حال بارگذاری...</p>';

        // Apply filters for custom content loading
        $content = apply_filters('psych_interactive_modal_content', $content, $m
odal_id, $user_id, $context);
    ...
    public function handle_station_completion($user_id, $node_id, $station_data)
 {
        ...
        do_action('psych_interactive_station_completed', $user_id, $node_id, $st
ation_data);
    }
...
.psych-modal-overlay.active {
    ...
    <!-- Modal Container -->
    <div id="psych-modal-overlay" class="psych-modal-overlay" style="display: no
ne;">
        <div class="psych-modal-dialog">
            <button class="psych-modal-close" id="psych-modal-close">&times;</bu
tton>
    ...
    jQuery(document).ready(function($) {
        ...
        function loadModalContent(modalId) {
            modalContent.html('<div style="text-align:center; padding:40px;"><i
class="fas fa-spinner fa-spin" style="font-size:2rem; color:#3498DB;"></i><p>در
حال بارگذاری...</p></div>');
            modal.show().addClass('active');
            $('body').css('overflow', 'hidden');
            ...
            if (typeof psych_interactive_ajax === 'undefined') {
                modalContent.html('<div class="alert alert-danger"><p>خطا: متغی
رهای JavaScript بارگذاری نشده‌اند.</p></div>');
                return;
            }
            ...
            $.post(psych_interactive_ajax.ajax_url, {
                ...
                if (response.success) {
                    ...
                    modalContent.html('<div class="alert alert-danger"><p>خطا د
ر بارگذاری محتوا: ' + (response.data.message || 'خطای ناشناخته') + '</p></div>')
;
                }
            }).fail(function(xhr, status, error) {
                modalContent.html('<div class="alert alert-danger"><p>خطای ارتب
اط با سرور: ' + error + '</p></div>');
            });
        }
        ...
        function completeMission(missionId, buttonElement) {
            ...
            buttonElement.prop('disabled', true).html('<i class="fas fa-spinner
fa-spin"></i> در حال پردازش...');

            // Here you would integrate with the path engine or other mission sy
stems
            // For now, we'll show a success message
            setTimeout(function() {
                alert('✅ ماموریت با موفقیت تکمیل شد!');
                buttonElement.prop('disabled', false).html('✅ تکمیل شد');
                buttonElement.removeClass('psych-btn-primary').addClass('psych-b
tn-success');

                // Fire custom event for other integrations
                $(document).trigger('psych_mission_completed', [missionId, butto
nElement]);
            }, 1500);
        }
