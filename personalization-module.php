 ...
 * Description: A module to provide a powerful conditional shortcode [psych_pers
onalize] for showing/hiding content based on user data, quiz results, and more.
 * Version: 1.0.0
 * Author: Jules
 */
...
final class Psych_Personalization_Module {
    ...
    public function register_shortcode() {
        add_shortcode('psych_personalize', array($this, 'render_personalize_shor
tcode'));
    }
    ...
    private function evaluate_conditions($condition_string) {
        ...
        $parts = preg_split('/(>=|<=|!=|=|>|<)/', $condition_string, -1, PREG_SP
LIT_DELIM_CAPTURE);
        if (count($parts) !== 3) {
        ...
        $context = function_exists('psych_get_viewing_context') ? psych_get_view
ing_context() : ['viewed_user_id' => get_current_user_id()];
        ...
        switch ($source) {
            ...
            case 'quiz':
                ...
                $score = $wpdb->get_var($wpdb->prepare(
                    "SELECT score FROM {$table_name} WHERE user_id = %d AND quiz
_id = %s ORDER BY id DESC LIMIT 1",
                    $user_id, $source_id
                ));
                $actual_value = $score;
                break;
            ...
            case 'role':
                ...
                if ($user && in_array($source_id, (array) $user->roles, true)) {
                    $actual_value = $source_id; // For role checks, we just need
 to match the role name
                }
                break;
            ...
            case 'points':
                if (function_exists('psych_get_user_points')) {
                    ...
                    $actual_value = get_user_meta($user_id, 'psych_total_points'
, true);
                }
                break;
            ...
            case 'badge':
                 if (function_exists('psych_user_has_badge')) {
                    // This source just checks for existence of a badge by its s
lug or ID
                    if (psych_user_has_badge($user_id, $source_id)) {
                        $actual_value = 'true'; // Set to string 'true' for comp
arison
                    }
                }
                break;
