<?php
/**
 * Plugin Name: Psych Complete System - Report Card (Enhanced Integration Editio
n)
...
final class Psych_Unified_Report_Card_Enhanced {
    ...
    private function add_hooks() {
        ...
        add_action('wp_ajax_psych_save_user_notes', [$this, 'ajax_save_user_note
s']);
        add_action('wp_ajax_psych_save_user_goals', [$this, 'ajax_save_user_goal
s']);
        add_action('wp_ajax_psych_send_parent_report', [$this, 'ajax_send_parent
_report']);
    ...
    public function enqueue_assets() {
        global $post;
        if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'psych_
report_card')) {
            wp_enqueue_script('jquery');
            wp_enqueue_script('chart-js', 'https://cdn.jsdelivr.net/npm/chart.js
', [], '3.9.1', true);

            wp_enqueue_script('psych-report-card', false, ['jquery', 'chart-js']
, self::VERSION, true);
    ...
    public function print_styles() {
        ...
        <style>
            .psych-report-card-container {
                font-family: 'Vazirmatn', -apple-system, BlinkMacSystemFont, 'Se
goe UI', Roboto, sans-serif;
            ...
            .psych-report-card-container::before {
                ...
                width: 200%;
                height: 200%;
                background: radial-gradient(circle, rgba(255,255,255,0.1) 0%, tr
ansparent 70%);
                pointer-events: none;
            }
...
;
    ...
    public function get_inline_js() {
        ...
        window.PsychReportCard = {
            ...
            initTabs: function() {
                jQuery(".psych-report-card-tabs .tab-link").on("click", function
(e) {
                    e.preventDefault();
                    var targetTab = jQuery(this).data("tab");

                    // Update active tab
                    jQuery(".psych-report-card-tabs .tab-link").removeClass("act
ive");
...
{}");
                    ...
                    new Chart(badgesCtx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                data: [badgesData.earned || 0, badgesData.remain
ing || 0],
            ...
            initNotesAutoSave: function() {
                var saveTimeout;
                jQuery("#psych-user-notes, #psych-user-goals").on("input", funct
ion() {
            ...
            saveUserData: function($element) {
                var fieldType = $element.attr("id") === "psych-user-notes" ? "no
tes" : "goals";
                ...
                jQuery.post(psych_report_card.ajax_url, {
                    ...
                    if (response.success) {
                        ...
                        $element.after("<span class=\"save-indicator\" style=\"c
olor: #27ae60; font-size: 12px; margin-right: 10px;\">✓ ذخیره شد</span>");
                        setTimeout(() => {
            ...
            sendParentReport: function() {
                ...
                jQuery("#send-parent-report").prop("disabled", true).text("در ح
ال ارسال...");
                ...
                jQuery.post(psych_report_card.ajax_url, {
                    ...
                    if (response.success) {
                        ...
                        alert("خطا در ارسال گزارش: " + (response.data?.message |
| "خطای نامشخص"));
                    }
                }).fail(function() {
                    alert("خطا در ارتباط با سرور");
                }).always(function() {
                    jQuery("#send-parent-report").prop("disabled", false).text("
ارسال گزارش");
                });
            }
        };
...
']);
        add_shortcode('psych_user_summary', [$this, 'render_user_summary_shortco
de']);
        add_shortcode('psych_path_progress', [$this, 'render_path_progress_short
code']);
    }
...
>';
        ...
        ob_start();
        ?>
        <div class="psych-report-card-container <?php echo $is_compact ? 'compac
t' : ''; ?>">
            <?php if ($context['is_impersonating']): ?>
                <div class="psych-impersonation-notice">
                    <i class="fas fa-eye"></i>
                    شما در حال مشاهده کارنامه <strong><?php echo esc_html($user-
>display_name); ?></strong> هستید
                </div>
            <?php endif; ?>
    ...
    private function render_header($user, $context) {
        ...
        <div class="psych-report-card-header">
            <img src="<?php echo esc_url($avatar_url); ?>" alt="آواتار" class="a
vatar">
            <div class="user-info">
                <h2><?php echo esc_html($user->display_name); ?></h2>
                <div class="psych-user-status">
                    <span class="level" style="color: <?php echo esc_attr($level
_info['color']); ?>">
                        <i class="<?php echo esc_attr($level_info['icon']); ?>">
</i>
                        <?php echo esc_html($level_info['name']); ?>
                    </span>
                    •
                    <span class="points"><?php echo number_format_i18n($total_po
ints); ?> امتیاز</span>
                    •
                    <span class="join-date">عضویت: <?php echo wp_date('j F Y', s
trtotime($user->user_registered)); ?></span>
                </div>
                <?php if ($level_info['points_to_next'] > 0): ?>
                    <div class="psych-progress-bar" data-percent="<?php echo esc
_attr($level_info['progress_percentage']); ?>">
                        <div class="psych-progress-fill"></div>
                    </div>
                    <small><?php echo number_format_i18n($level_info['points_to_
next']); ?> امتیاز تا سطح بعدی</small>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    private function render_tabs($tabs) {
        $tab_labels = [
            'overview' => ['title' => 'نمای کلی', 'icon' => 'fas fa-tachometer-a
lt'],
...
'],
 ...
 ''; ?>"
                        ...
                        <i class="<?php echo esc_attr($tab_labels[$tab]['icon'])
; ?>"></i>
    ...
    private function render_tab_contents($user_id, $tabs, $context) {
        foreach ($tabs as $index => $tab) {
            $is_active = $index === 0;
            echo '<div id="psych-tab-' . esc_attr($tab) . '" class="psych-tab-co
ntent' . ($is_active ? ' active' : '') . '">';
        ...
        <div class="psych-section">
            ...
            <div class="psych-summary-grid">
                <div class="psych-summary-item">
                    <div class="value"><?php echo number_format_i18n($summary_da
ta['total_points']); ?></div>
...
ta['badges_count']); ?></div>
                ...
                <div class="psych-summary-item">
                    <div class="value"><?php echo number_format_i18n($summary_da
ta['completed_stations']); ?></div>
                    <div class="label">ایستگاه‌های تکمیل شده</div>
                </div>
                <div class="psych-summary-item">
                    <div class="value"><?php echo $summary_data['progress_percen
tage']; ?>%</div>
        ...
        <div class="psych-section">
            ...
            <div class="psych-achievements-list">
                <?php foreach ($summary_data['recent_achievements'] as $achievem
ent): ?>
                    <div class="psych-achievement-item">
                        <i class="<?php echo esc_attr($achievement['icon']); ?>"
 style="color: <?php echo esc_attr($achievement['color']); ?>"></i>
                        <span><?php echo esc_html($achievement['title']); ?></sp
an>
                        <small><?php echo human_time_diff(strtotime($achievement
['date'])); ?> پیش</small>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <?php if ($context['is_impersonating'] || current_user_can('manage_optio
ns')): ?>
        <div class="psych-section">
            <h3><i class="fas fa-sms"></i> ارسال گزارش به والدین</h3>
            <div class="psych-parent-report">
                <input type="text" id="parent-mobile" placeholder="شماره موبایل
والدین"
                       value="<?php echo esc_attr(get_user_meta($user_id, self::
PARENT_MOBILE_META, true)); ?>">
                <button type="button" class="psych-button" onclick="PsychReportC
ard.sendParentReport()">
                    ارسال گزارش
                </button>
            </div>
        </div>
                    ...
                    <?php foreach ($badges_data['earned'] as $badge): ?>
                        <div class="psych-badge-item">
                            <i class="<?php echo esc_attr($badge['icon']); ?>"
                               ...
                               style="color: <?php echo esc_attr($badge['color']
); ?>"></i>
                            <p class="badge-name"><?php echo esc_html($badge['na
me']); ?></p>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <?php foreach ($badges_data['available'] as $badge): ?>
                    <div class="psych-badge-item" style="opacity: 0.6;">
                        <i class="<?php echo esc_attr($badge['icon']); ?>"
                           style="color: <?php echo esc_attr($badge['color']); ?
>"></i>
                        <p class="badge-name"><?php echo esc_html($badge['name']
); ?></p>
                        <small><?php echo esc_html($badge['description']); ?></s
mall>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="psych-section">
            ...
            <div class="psych-user-rank">
                <p>رتبه شما: <strong><?php echo number_format_i18n($leaderboard_
data['user_rank']); ?></strong> از <?php echo number_format_i18n($leaderboard_da
ta['total_users']); ?> کاربر</p>
            </div>
            ...
            <table class="psych-leaderboard-table">
                ...
                <tbody>
                    <?php foreach ($leaderboard_data['top_users'] as $index => $
user_data): ?>
                        <tr <?php echo $user_data['ID'] == $user_id ? 'class="cu
rrent-user"' : ''; ?>>
        ...
        <div class="psych-section">
            ...
            <?php if (!empty($path_data['current_path'])): ?>
                <div class="psych-path-info">
                    <h4>مسیر فعلی: <?php echo esc_html($path_data['current_path'
]['title']); ?></h4>
                    <p><?php echo esc_html($path_data['current_path']['descripti
on']); ?></p>

                    <div class="psych-progress-bar" data-percent="<?php echo esc
_attr($path_data['completion_percentage']); ?>">
                        <div class="psych-progress-fill"></div>
                    </div>
                    <p><?php echo $path_data['completion_percentage']; ?>% تکمی
ل شده
                       (<?php echo $path_data['completed_stations']; ?> از <?php
 echo $path_data['total_stations']; ?> ایستگاه)</p>
                </div>
            <?php endif; ?>
            ...
            <div class="psych-path-progress">
                <?php if (!empty($path_data['stations'])): ?>
                    <?php foreach ($path_data['stations'] as $station): ?>
                        <div class="psych-path-station <?php echo esc_attr($stat
ion['status']); ?>">
 ...
 ?>"></i>
                            ...
                            <div class="psych-station-details">
                                <h4><?php echo esc_html($station['title']); ?></
h4>
                                <p><?php echo esc_html($station['description']);
 ?></p>
                                <?php if ($station['status'] === 'completed' &&
$station['completion_date']): ?>
                                    <small>تکمیل شده در: <?php echo wp_date('j F
 Y', strtotime($station['completion_date'])); ?></small>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class="psych-alert info">هنوز مسیری انتخاب نشده است.</d
iv>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="psych-section">
            ...
            <div class="psych-chart-container">
                <canvas id="psych-progress-chart"
                        data-chart-data="<?php echo esc_attr(json_encode($analyt
ics_data['progress_chart'])); ?>"></canvas>
            </div>
        </div>
...
?>
                        <tr>
                            ...
                            <td><?php echo wp_date('j F Y', strtotime($result['d
ate'])); ?></td>
                            <td class="psych-score-cell <?php echo esc_attr($res
ult['score_class']); ?>">
    ...
    private function render_notes_tab($user_id, $context) {
        $can_edit = ($context['real_user_id'] == $user_id) || current_user_can('
manage_options');
        ...
        <div class="psych-section">
            ...
            <?php if ($can_edit): ?>
                <div class="psych-notes-section">
                    <textarea id="psych-user-notes" placeholder="یادداشت‌های خو
د را اینجا بنویسید..."><?php echo esc_textarea($user_notes); ?></textarea>
                </div>
            <?php else: ?>
                <div class="psych-notes-display">
                    <?php echo $user_notes ? nl2br(esc_html($user_notes)) : '<em
>یادداشتی وجود ندارد.</em>'; ?>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="psych-section">
            ...
            <?php if ($can_edit): ?>
                <div class="psych-notes-section">
                    <textarea id="psych-user-goals" placeholder="اهداف و برنامه
‌های خود را اینجا تعریف کنید..."><?php echo esc_textarea($user_goals); ?></texta
rea>
                </div>
            <?php else: ?>
                <div class="psych-notes-display">
                    <?php echo $user_goals ? nl2br(esc_html($user_goals)) : '<em
>هدفی تعریف نشده است.</em>'; ?>
                </div>
            <?php endif; ?>
        </div>
        <?php
    ...
    private function get_user_badges_data($user_id) {
        ...
        if (function_exists('psych_gamification_get_instance')) {
            ...
            if (method_exists($gamification, 'get_user_badges_progress')) {
                $badges_progress = $gamification->get_user_badges_progress($user
_id);
    ...
    private function get_leaderboard_data($user_id) {
        ...
        if (class_exists('Psych_Gamification_Center')) {
            ...
            if (method_exists($gamification, 'get_user_rank')) {
                $leaderboard_data['user_rank'] = $gamification->get_user_rank($u
ser_id);
            }

            if (method_exists($gamification, 'get_top_users_by_points')) {
                $leaderboard_data['top_users'] = $gamification->get_top_users_by
_points(10);
            }
        }
    ...
    private function get_user_path_data($user_id) {
        ...
        if (function_exists('psych_path_get_user_progress')) {
            return array_merge($default_data, psych_path_get_user_progress($user
_id));
        }
    ...
    private function get_user_test_results($user_id) {
        $test_results = get_user_meta($user_id, self::TEST_RESULTS_META_KEY, tru
e) ?: [];
...
id) {
    ...
    public function ajax_save_user_notes() {
        ...
        if (!$user_id || ($context['real_user_id'] != $user_id && !current_user_
can('manage_options'))) {
...
_info['color']); ?>">
                    ...
                    <span class="points">
                        <?php echo number_format_i18n($summary_data['total_point
s']); ?> امتیاز
                    </span>
                    <span class="badges">
                        <?php echo number_format_i18n($summary_data['badges_coun
t']); ?> نشان
                    </span>
                </div>

                <?php if ($atts['show_progress'] === 'true' && $level_info['poin
ts_to_next'] > 0): ?>
                    <div class="psych-progress-bar" data-percent="<?php echo esc
_attr($level_info['progress_percentage']); ?>">
                        <div class="psych-progress-fill"></div>
...
next']); ?> امتیاز تا سطح بعدی</small>
    ...
    public function render_path_progress_shortcode($atts) {
        ...
        if (empty($path_data['current_path'])) {
            return '<div class="psych-alert info">هنوز مسیری انتخاب نشده است.</d
iv>';
        }
...
' : ''; ?>">
            <div class="psych-path-header">
                <h4><?php echo esc_html($path_data['current_path']['title']); ?>
</h4>
                <div class="psych-progress-bar" data-percent="<?php echo esc_att
r($path_data['completion_percentage']); ?>">
                    <div class="psych-progress-fill"></div>
                </div>
                <p><?php echo $path_data['completion_percentage']; ?>% تکمیل شد
ه</p>
            </div>

            <?php if (!$is_compact && !empty($path_data['stations'])): ?>
                <div class="psych-path-stations">
                    <?php foreach (array_slice($path_data['stations'], 0, 5) as
$station): ?>
                        <div class="psych-station-mini <?php echo esc_attr($stat
ion['status']); ?>">
                            <i class="<?php echo esc_attr($station['icon']); ?>"
></i>
                            <span><?php echo esc_html($station['title']); ?></sp
an>
                        </div>
                    <?php endforeach; ?>
                </div>
...
register_activation_hook(__FILE__, function() {
    ...
    if (!get_option('psych_report_card_version')) {
        update_option('psych_report_card_version', Psych_Unified_Report_Card_Enh
anced::VERSION);
        ...
        // update_user_meta($user_id, Psych_Unified_Report_Card_Enhanced::TEST_R
ESULTS_META_KEY, $sample_results);
    }
});
...
if (!function_exists('psych_add_test_result')) {
    ...
    function psych_add_test_result($user_id, $test_title, $score) {
        $test_results = get_user_meta($user_id, Psych_Unified_Report_Card_Enhanc
ed::TEST_RESULTS_META_KEY, true) ?: [];
        ...
        update_user_meta($user_id, Psych_Unified_Report_Card_Enhanced::TEST_RESU
LTS_META_KEY, $test_results);
        ...
        wp_add_dashboard_widget(
            ...
            function() {
                ...
                <div class="psych-dashboard-widget">
                    <div class="psych-stats-grid" style="display: grid; grid-tem
plate-columns: repeat(3, 1fr); gap: 15px; text-align: center;">
                        <div class="stat-item">
                            <h3 style="margin: 0; color: #3498db;"><?php echo nu
mber_format_i18n($total_users); ?></h3>
                            <p style="margin: 5px 0 0 0; color: #666;">کل کاربر
ان</p>
                        </div>
                        <div class="stat-item">
                            <h3 style="margin: 0; color: #27ae60;"><?php echo nu
mber_format_i18n($active_paths); ?></h3>
                            <p style="margin: 5px 0 0 0; color: #666;">مسیرهای
فعال</p>
                        </div>
                        <div class="stat-item">
                            <h3 style="margin: 0; color: #f39c12;"><?php echo nu
mber_format_i18n($total_badges_awarded); ?></h3>
                            <p style="margin: 5px 0 0 0; color: #666;">نشان‌های
اعطا شده</p>
                        </div>
                    </div>

                    <p style="margin-top: 15px; text-align: center;">
                        <a href="<?php echo admin_url('admin.php?page=psych-gami
fication-center'); ?>" class="button">
                            مشاهده جزئیات بیشتر
                        </a>
                    </p>
                </div>
...
add_action('rest_api_init', function() {
 ...
 $user_id) {
                return new WP_Error('unauthorized', 'دسترسی غیرمجاز', ['status'
=> 403]);
            }
...
=> 403]);
            ...
            if (empty($test_title) || $test_score < 0 || $test_score > 100) {
                return new WP_Error('invalid_data', 'داده‌های نامعتبر', ['status
' => 400]);
            }
            ...
            if ($result) {
                ...
                return new WP_Error('save_failed', 'خطا در ثبت نتیجه', ['status'
 => 500]);
            }
        },
        'permission_callback' => function() {
...
if (defined('WP_DEBUG') && WP_DEBUG) {
    add_action('psych_report_card_loaded', function() {
        error_log('Psych Report Card Enhanced Integration Edition loaded success
fully');
    });
}
